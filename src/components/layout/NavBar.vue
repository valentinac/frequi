  ...
  <header>
    <b-navbar toggleable="sm" dark variant="primary">
      <router-link class="navbar-brand" exact to="/">
        ...
        <span class="navbar-brand-title d-sm-none d-md-inline">FaiTrader</span>
      </router-link>
      ...
      <b-collapse id="nav-collapse" class="text-center" is-nav>
        <b-navbar-nav>
          <b-nav-item v-if="!botStore.canRunBacktest" to="/trade">订单</b-nav-item>
          <b-nav-item v-if="!botStore.canRunBacktest" to="/dashboard">控制面板</b-nav-item>
          <b-nav-item to="/graph">图表</b-nav-item>
          <b-nav-item to="/logs">日志</b-nav-item>
          <b-nav-item v-if="botStore.canRunBacktest" to="/backtest">回测</b-nav-item>
          <b-nav-item
            v-if="
        ...
        <b-navbar-nav class="ms-auto" menu-class="w-100">
          ...
          <div class="d-flex justify-content-between">
            ...
            <ReloadControl class="me-3" title="确认对话框停用." />
          </div>
          <li
          ...
          <li v-if="botStore.hasBots" class="nav-item">
            ...
            <b-nav-item-dropdown id="avatar-drop" right auto-close class="d-none d-sm-block">
              <template #button-content>
                <b-avatar size="2em" button>Fai</b-avatar>
              </template>
              ...
              <b-dropdown-item active-class="non-existant" to="/settings">系统设置</b-dropdown-item>
              <div class="ps-3">
                <b-form-checkbox v-model="layoutStore.layoutLocked">锁定布局</b-form-checkbox>
              </div>
              <b-dropdown-item @click="resetDynamicLayout">重置布局</b-dropdown-item>
              <b-nav-item
                v-if="botStore.botCount === 1"
                class="dropdown-item"
                to="/"
            ...
            <div class="d-block d-sm-none">
              ...
              <b-nav-item class="py-0" to="/settings" title="系统设置">
                系统设置 <i-mdi-cog class="ms-auto" />
              </b-nav-item>
              <b-nav-item
                v-if="botStore.botCount === 1"
          ...
          <li v-else>
            ...
            <LoginModal v-if="route?.path !== '/login'" title="登录" />
          </li>
        </b-navbar-nav>
      </b-collapse>
    </b-navbar>
